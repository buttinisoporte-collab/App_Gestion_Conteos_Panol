...
import { WeekData, Item, AuditLogEntry } from '../types';
...
const ConteoTable: React.FC<ConteoTableProps> = ({ week, onBack, onPrint }) => {
  ...
  const confirmFinalize = () => {
    ...
    // Consider adding a success toast/modal here instead of alert
  };
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 bg-white/80 backdrop-blur-sm py-4 -mx-4 px-4 sticky top-0 z-10 border-b">
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          <Button onClick={() => setView('report')} variant="secondary" className="w-full sm:w-auto">
            Ver Reporte Semanal
          </Button>
          <Button onClick={() => onPrint({ ...week, items: editedItems })} variant="secondary" className="w-full sm:w-auto">
            Descargar Planilla
          </Button>
      ...
      {showFinalizeModal && (
        <AlertDialog
          ...
          description="Esta acción no se puede deshacer. No podrá realizar más cambios en este conteo."
      ...
      <div className="flex items-center mb-4">
        <Input 
          placeholder="Buscar por ID Material..."
          value={searchTerm}
              ...
              <TableRow key={item.id}>
                ...
                <TableCell className="text-center">
                   <Input
                    ...
                    onChange={(e) => handleItemChange(item.id, 'systemStock', e.target.valueAsNumber || 0)}
                    disabled={!canEdit}
                    className="w-24 text-center font-bold mx-auto bg-slate-50"
                    min="0"
                  />
                </TableCell>
                <TableCell className="text-center">
